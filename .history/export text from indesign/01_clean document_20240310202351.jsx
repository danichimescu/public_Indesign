...
function cleanDocument() {
    ...
    var savedoc = app.activeDocument.save(
        File(myFile_path + "/" + myFile_nameNoeExt + "_workBD_Raw.indd")
    );
    var doc = app.activeDocument;
    ...
    $.sleep(1000);
    try {
        var myLayerPrices = doc.layers.add({ name: "newLayer_Prices_work" });
    } catch (e) {
        myLayerPrices = doc.layers.item("newLayer_Prices_work");
    }

    try {
        var myLayerBack = doc.layers.add({ name: "newLayer_Background_work" });
    } catch (e) {
        myLayerBack = doc.layers.item("newLayer_Background_work");
    }

    try {
        var myLayer_images = doc.layers.add({ name: "newLayer_Images" });
    } catch (e) {
        myLayer_images = doc.layers.item("newLayer_Images");
    }

    try {
        var myLayer = doc.layers.add({ name: "newLayer_TextFrames" });
    } catch (e) {
        myLayer = doc.layers.item("newLayer_TextFrames");
    }
        ...
        for (var j = 0; j < itemsOnPage.length; j++) {
            ...
            if (myTextFrame instanceof TextFrame) {
                if (myTextFrame.words.length === 0) {
                    ...
                    progress.display(
                        "Remove empty textframes on page " + sourcePages[i].name
                    );
                    myTextFrame.remove();
                }
            }
    ...
    for (var i = 0; i < app.activeDocument.spreads.length; i++) {
        //---------------- text
        var item,
            myPageItemsText = app.activeDocument.textFrames;
        ...
        //---------------- images
        var itemI,
            myPageItemsImages = app.activeDocument.rectangles;
        ...
        for (var i = 0; i < myPageItemsImagesOvals.length; i++) {
            if (
                myPageItemsImagesOvals[i].images.length >= 1 ||
                myPageItemsImagesOvals[i].rectangles.length >= 1
            )
        ...
        for (var i = 0; i < myPageItemsImagesPoly.length; i++) {
            if (
                myPageItemsImagesPoly[i].images.length >= 1 ||
                myPageItemsImagesPoly[i].rectangles.length >= 1
            )
        ...
        while ((itemI = arrayImagesFrames.shift())) {
            ...
            try {
                itemI.itemLayer = myLayer_images;
            } catch (e) { } //alert(e)
            progress.increment();
        }
    }
    ...
    for (var i = 0; i < sourcePages.length; i++) {
        ...
        var targetLayer = app.activeDocument.layers.itemByName(
            "newLayer_Background_work"
        );
        ...
        var rect = sourcePages[i].rectangles.add({
            geometricBounds: [0, 0, b_pgebounds[2], b_pgebounds[3]],
        });
            ...
            if (myTextFrame instanceof TextFrame) {
                ...
                if (myTextFrame.contents !== "") {
                    ...
                    if (myTextFrame.words[0]) {
                        ...
                        if (word.fontStyle !== null && word.fontStyle === "Black") {
                            //&& word.item(0).pointSize > 20
                            progress.display("Move prices on page " + sourcePages[i].name);
                            try {
                                myTextFrame.itemLayer = myLayerPrices;
                            } catch (e) {
                                alert(e);
                            }
                        }
    ...
    var tobeDeleted = [
        "379353_Shutterstock_2175025063_macro-beer-bottle-textureWater-drops-texture-on-the-bottle-of-beer-Abstract-backgroundWater-drops-ba.jpg",
        "morzsi 10 cm 300 dpi cmyk.psd",
        "model_traditional.ai",
        "Logo_PENNY_READY.ai",
        "stopy.tif",
        "drops.tif",
        "macelar.psb",
        "PE_RDY_Papierstruktur_CMYK.psd",
        "PE_RDY_Wischer_SW_groß.tif",
        "icon proaspat copt 1.psd",
        "EU-Ecolabel_EDS RO.pdf",
        "plenty-of-watering.ai",
        "Dairy_2_alb.png",
    ];
    var links = app.activeDocument.links;
    var link, image;
    ...
    function OLD_SLOW_removeATextFrameWhichStartWithaWord(myText) {
        //valabilitate
    ...
    var savedocDoi = app.activeDocument.save(
        File(myFile_path + "/" + myFile_nameNoeExt + "_workBD_final.indd")
    );
