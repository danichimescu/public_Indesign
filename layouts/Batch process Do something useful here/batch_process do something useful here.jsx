...
function get_data(history) {
	...
	var w = new Window('dialog', 'Batch process do something useful', undefined, { closeButton: false });
	...
	var infolder = folder.add('group {_: StaticText {text: "Indd/txt Input folder:"}}');
	...
	//write_Dimensions
	var write_Dimensions = miscOptions1.add('checkbox', undefined, "Write report W H (total) and w h (visible)");
	var write_effectivePPI = miscOptions1.add('checkbox', undefined, "Write report Effective PPI");
	...
	if (w.show() == 2) { // 2 inseamna cancel
		var dlg_data = collect_dlg_data();
		if (save_settings.value) {
			store_settings(history, dlg_data);
			// Need to store the preset, it overwrites the default presets.
			store_settings(File(script_dir() + batch_presets.selection.text.replace(/[\[\]]/g, '') + '.batch_process'), dlg_data);
		}
		w.close();
	...
	function get_target_extension(s) {
		switch (s) {
			...
			case 'Tagged text': return 'TAGGED_TEXT';
			case 'PDF (Interactive)': return 'PDF';
			case 'AEM (Adobe Experience Manager)': return 'AEM';
			default: return s;
		}
	}
...
function writeEffectivePpi(current_doc) {
	...
	effectivePPiarray.push([fileName, imageName, myefectivePPi[0], myefectivePPi[1]]);
...
function writeEffectivePPi_file(effectivePPiarray, params) {
	for (var a = 0; a < effectivePPiarray.length; a++) {
		...
		var filename = "_indesign_raport_effectivePPi_" + dateString_ziua + "_" + ".txt"; // merge!
		// alert("filename " + filename)
